...
var SSPInstance = Class.extend({
	...
	initialize: function(opts) {
		...
		var options = (this.options = {
			...
			parity:
				opts.parity &&
				['even', 'mark', 'odd', 'space'].indexOf(opts.parity.toString().toLowerCase()) > -1
					? opts.parity
					: 'none',
		...
		if (this.port && this.port.isOpen) {
			...
			SerialPort.list(function(err, ports) {
				if (err || ports.length === 0) {
					...
					for (var i in ports) {
						if (
							ports[i].vendorId === '0x191c' ||
							(ports[i].pnpId && ports[i].pnpId.indexOf('Innovative_Technology') > -1)
						) {
		...
		function initializeDevice() {
			...
			port.open(function(err) {
				...
				if (err) {
					...
					// temporary buffer to hold the incoming data
					var tempBuffer;
					port.on('data', function(buffer) {
						// store the incoming buffer
						if (!tempBuffer) tempBuffer = buffer;
						else tempBuffer = Buffer.concat([tempBuffer, buffer]);
						// if we have enough data to calculate the length of the incoming frame
						if (tempBuffer.length > 2) {
							var len = tempBuffer[2] + 5;
							// check if we have enough data to extract the incoming frame
							if (tempBuffer.length >= len) {
								...
								do {
									...
									tempBuffer.copy(buf, 0, ix, ix + len);
									parseBuffer(buf);
									ix += len;
								} while (ix < tempBuffer.length);
								// after the parsing, reset the temporary buffer
